...
export async function getDashboardStats() {
  ...
  try {
    const [
      ...
      employersResult,
      activeEventsResult,
      securityPersonnelResult,
      onlineUsersResult,
      todayCheckinsResult,
      todayIncidentsResult
    ] = await Promise.all([
      ...
      db.select({ count: count() }).from(securityPersonnel),
      db.select({ count: count() }).from(users).where(eq(users.isOnline, true)),
      db.select({ count: count() }).from(attendanceRecords).where(gte(attendanceRecords.checkInTime, new Date(Date.now() - 24 * 60 * 60 * 1000))),
      db.select({ count: count() }).from(securityIncidents).where(gte(securityIncidents.createdAt, new Date(Date.now() - 24 * 60 * 60 * 1000)))
    ]);
    ...
    return {
      ...
      securityPersonnel: securityPersonnelResult[0]?.count || 0,
      onlineUsers: onlineUsersResult[0]?.count || 0,
      checkinsToday: todayCheckinsResult[0]?.count || 0,
      incidentsToday: todayIncidentsResult[0]?.count || 0,
      systemHealth: 98, // This would come from actual health checks
      attendanceRate: 85 // This would be calculated from actual data
    };
  } catch (error) {
...
export async function getAllUsers(filters?: {
  ...
  try {
    ...
    if (filters?.search) {
      ...
      return result.filter((user: typeof result[0]) => 
...
export async function getAllEmployers(filters?: {
  ...
  try {
    ...
    const result = conditions.length > 0
      ? await query.where(and(...conditions)).orderBy(desc(employers.createdAt))
      : await query.orderBy(desc(employers.createdAt));

    // Get job counts for each employer
    const employersWithStats = await Promise.all(
      result.map(async (employer: typeof result[0]) => {
        const [jobsCount, interviewsCount] = await Promise.all([
          db.select({ count: count() }).from(jobs).where(eq(jobs.employerId, employer.id)),
        ...
        return {
          ...employer,
          jobsPosted: jobsCount[0]?.count || 0,
          interviewsScheduled: interviewsCount[0]?.count || 0,
          status: employer.userIsActive ? 
            (employer.isVerified ? "active" : "pending") : "inactive"
        };
      })
    );
    ...
    if (filters?.search) {
      const searchTerm = filters.search.toLowerCase();
      return employersWithStats.filter((employer: typeof employersWithStats[0]) => 
        employer.companyName?.toLowerCase().includes(searchTerm) ||
        employer.contactPerson?.toLowerCase().includes(searchTerm) ||
        employer.userEmail?.toLowerCase().includes(searchTerm)
      );
    }
